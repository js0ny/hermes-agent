...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    # then fall back to external dirs and user skills dir from config.
    try:
        from agent.skill_utils import get_all_skills_dirs
        dirs_to_check = [d.resolve() for d in get_all_skills_dirs()]
    except Exception:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    # Scan local dir first, then user dir, then external dirs (local takes precedence)
